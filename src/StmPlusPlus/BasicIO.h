...
/**
 * @brief Template that implements a data message.
 */
template <size_t N> class DataMessage
{
private:
...
public:
    ...
    DataMessage (uint32_t v):
        radix(16),
        size(N)
    {
        for(size_t i = 0; i < size; ++i) data[i] = v;
        bufferSize = size*8 + size + 2;
        bufferPtr = &buffer[0];
    }

    inline void setRadix (int r)
    {
        radix = r;
    }
    ...
    char * buildBuffer (bool addBrackets = true)
    {
        ::memset(bufferPtr, '0', bufferSize);
        ...
        {
            ::__itoa(data[i], digits, radix);
            size_t len = ::strlen(digits);
            ::memcpy(msgPtr + i*9 + 8 - len, digits, len);
            if (i < size -1)
            {
                msgPtr[i*9 + 8] = ';';
            }
        ...
        if (addBrackets)
        {
            bufferPtr[bufferSize - 2] = ']';
            bufferPtr[bufferSize - 1] = 0;
        }
        else
        {
            bufferPtr[bufferSize - 3] = 0;
        }
        return bufferPtr;
    }
    ...
    void parseBuffer ()
    {
        ...
        for (size_t i = 0; i < size; ++i)
        {
            ::memcpy(digits, msgPtr + i*9, 8);
            data[i] = ::strtoll(digits, NULL, radix);
        }
    }
